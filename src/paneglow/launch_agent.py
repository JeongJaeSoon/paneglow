...
foreground ``run`` command; it must never run ``start``, which detaches a child
...
LABEL = "io.github.jeongjaesoon.paneglow"
...
def build_spec(
    *,
    ...
    if len(command) != 3 or command[1:] != ("-m", "paneglow.cli"):
        raise LaunchAgentError("unsupported Paneglow command prefix")
    ...
    configured_runtime = environment.get(
        "PANEGLOW_HOME", str(login_home / ".paneglow")
    )
    ...
    return Spec(
        ...
        # not split when PANEGLOW_HOME changes.
...
def _generated_manifest(value: object, *, account_home: Path) -> bool:
    ...
    if not isinstance(program, str) or not Path(program).is_absolute():
        return False
    if arguments != [program, "-m", "paneglow.cli", "run"]:
        return False
    ...
    runtime_home = environment.get(
        "PANEGLOW_HOME", str(account_home / ".paneglow")
    )
...
def _check_home_ancestors(spec: Spec) -> None:
    ...
    runtime_home = Path(spec.environment.get(
        "PANEGLOW_HOME", str(spec.account_home / ".paneglow")
    ))
...
def _ensure_runtime_directories(spec: Spec) -> None:
    ...
    # Older versions created ~/.paneglow as 0755. It is owner-controlled and
