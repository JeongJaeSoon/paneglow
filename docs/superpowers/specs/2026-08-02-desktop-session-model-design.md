...
## 조명 — 테두리 하나가 색과 움직임을 따로 쓴다

기존 설계는 테두리에 "화면 밖 대기 알림"을 넣었다. 그런데 우리는 벤더와 **같은 팔레트**를
일부러 쓰므로(주황=대기, 초록=완료…) **6키만 봐서는 Codex 것인지 Claude 것인지 구분되지 않는다.**

실측으로 `rgbcfg` 의 효과(`e`) 값이 다섯 종류 살아 있다는 것을 확인했으므로
([하드웨어 노트 §3](../../hardware-notes.md)), 테두리 한 존에 축을 둘 준다.
...
> 공장 팔레트 값이 더 잘 읽히고, "그 제품을 쓸 때 가장 자주 보는 색"이라는 점에서
> 의미도 맞는다.
...
효과는 **기기가 스스로 돌린다.** 메시지 한 번이면 끝이라 호스트 트래픽도, 데몬이 멈춰
애니메이션이 굳는 위험도 없다.

집계 대상은 슬롯에 못 오른 세션 전부이고, Codex를 볼 때는 **모든 Claude 세션**이다
(전부 화면 밖이므로). **움직임은 언제나 Claude 얘기다** — Codex 세션의 상태는 얻을 경로가
없다(범위 밖). 파란 테두리가 펄스하면 "6키는 Codex 것, 그런데 Claude가 너를 기다린다"는 뜻이다.

**C키 백라이트는 쓰지 않는다.** 한 존이 두 축을 쓰므로 필요가 없어졌다.
...
밀려난 세션과 7번째 이후 세션은 **테두리 움직임 집계**로 넘어간다 — 6키에도 테두리에도
...
테두리 **색**은 상태를 보지 않으므로 `render` 를 거치지 않고 게이트가 정한다.
...
| 노브 탭 순차 전환 (구 R7) | 위와 같음 |
| 세대(generation) · 입력 잠금 (구 §07) | 되돌릴 수 없는 동작이 없다 |
| 승인·거절 C2/C3 (구 설계 §04) | 유일한 수단이 Accessibility 권한이라 R6과 충돌 |
...
| **양보 중에 벤더가 테두리도 덮어쓰는가** | 테두리가 소유권과 경보를 모두 지므로 여기가 무너지면 둘 다 잃는다. 그때는 놀고 있는 C키 백라이트로 옮긴다 — 존만 바뀌고 색·움직임 어휘는 그대로 쓴다 |
