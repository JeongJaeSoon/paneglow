...
| R7 | 마지막으로 연 세션이 어느 키인지 알 수 있다 |
| R8 | 특별한 권한을 요구하지 않는다 |

**승인/거절(C2·C3)은 범위에서 제외한다.** 데스크톱 앱에는 세션에 응답을 보낼 수단이 없고,
남은 길은 Accessibility 권한으로 앱 UI를 찌르는 것뿐이라 R8과 정면으로 충돌한다.
...
| 존 | 명령 | 이 설계에서 |
|---|---|---|
| A1~A6 | `thstatus` | 세션별 상태 + 선택 표시. 양보 중에는 벤더 것 |
...
### A키 — 색은 상태, 펄스는 "지금 보고 있는 세션"

`thstatus` 는 **키마다 `e` 를 따로 받는다**(실측). 그래서 색을 건드리지 않고 한 키만 움직일 수 있다.

| A키 | 뜻 |
|---|---|
| 계속 켜짐 | 그 세션의 상태 (색) |
| **부드러운 펄스** (`e6`) | 그 세션의 상태 + **마지막으로 패드에서 연 세션** |
| 소등 | 빈 슬롯 |

**깜빡임(`e4`)은 쓰지 않는다.** 꺼져 있는 동안 상태 색이 사라져 선택된 세션의 상태만 읽을 수
없게 된다. 선택 표시는 오래 켜져 있는 표시라 그 손해가 계속 간다. 펄스는 밝기만 오르내린다.

C키도 펄스를 쓰지만 존이 달라 섞이지 않는다 — A키의 펄스는 "여기가 그거",
C키의 펄스는 "확인 필요"다.
...
### ④ 지금 보고 있는 세션 — 데이터 소스가 아니다

**우리가 연 장본인이므로 이미 알고 있다.** 딥링크를 쏜 세션을 데몬이 기억해두면 그만이고,
새로 읽을 것이 없다.

> **한계**: 앱 안에서 마우스로 세션을 바꾸면 우리는 모른다. 그래서 이 표시의 정확한 뜻은
> "마지막으로 **패드에서** 연 세션"이다. 근육 기억 관점에서는 그편이 오히려 맞고,
> 실사용에서 거슬리면 그때 `~/Library/Logs/Claude/main.log` 의
> `LocalSessions.setFocusedSession` 줄을 이어 읽어 보정한다 — 비공개 포맷이므로
> 필요해지기 전에는 만들지 않는다.

세션이 슬롯에서 밀려나거나 죽으면 표시도 같이 사라진다.
...
```jsonc
{
  ...
  "agent_keys": { "focused": "pulse" },           // pulse | off  -- 마지막으로 연 세션 표시
...
| 승인·거절 C2/C3 (구 설계 §04) | 유일한 수단이 Accessibility 권한이라 R8과 충돌 |
