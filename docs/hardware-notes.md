...
### 벤더와의 공존 (실측 · 2026-08-02 재측정)

- **채널 동시 open 가능** — Work Louder Input 앱이 실행 중이어도 우리가 벤더 채널을 열 수 있다.

초안의 "테두리는 60초간 유지됐다"는 **결론이 아니었다.** 그 시간 동안 벤더가 LED를 쓸 일이
있었는지 자체를 몰랐기 때문이다. 대조군을 넣어 다시 측정했다 — 6키를 벤더 팔레트에 없는
자홍(`#FF00FF`)으로 칠해두면, 그 색이 사라지는 것이 곧 "벤더가 이 시간에 LED를 썼다"는 증거다.

| 존 | 벤더가 건드리는가 | 방아쇠 |
|---|---|---|
| A1~A6 (`thstatus`) | **쓴다** | Codex 스레드 상태 변화 |
| 테두리 (`ambient`) | **쓴다** | **A키 입력** — 누른 스레드의 색을 테두리에 한 번 비춘다 |
| C1~C7 백라이트 (`keys`) | **안 건드린다** | — |

**테두리를 쓰는 방아쇠는 상태 변화가 아니라 사용자의 키 입력이다.** Codex에서 A키를 누르면
그 키가 파랑이었으면 테두리가 파랑, 초록이었으면 초록으로 한 번 바뀐다. 선택 피드백으로 보인다.

- **경쟁이 아니라 번갈아 쓰는 관계다.** 벤더는 이벤트마다 한 번 쏘고 붙들지 않는다.
  2초 간격으로 다시 쏘면 우리 색이 확실히 돌아온다(60초·30회 재전송으로 확인).
  대가는 벤더가 쓸 때마다 최대 2초짜리 잔상 하나.
- **C키 백라이트는 안전하다.** 같은 조건에서 자홍 펄스가 흔들림 없이 유지됐다.
  벤더와 겹치지 않는 신호를 두려면 여기가 맞다.

### 효과는 지속된다 · 우리 `thstatus`는 다른 존을 안 건드린다

- 테두리에 `e6`(펄스)를 걸고 **2분 이상** 방치해도 계속 펄스했다. 펌웨어가 효과를 되돌리지 않는다.
- 우리가 `thstatus`를 두 번 연달아 보내도 테두리는 그대로 펄스했다. **세 존 독립은 사실이며,
  위 표의 간섭은 순전히 벤더가 그 존에 쓰기 때문이다.**
